...
def xgbr(train, trainlabel, valid, validlabel):
    # XGB:
    regr = xgb.XGBRegressor(
        colsample_bytree=0.2,
        gamma=0.0,
        ...
        seed=42,
        silent=1)
    regr.fit(train, trainlabel)
    train_pred = regr.predict(train)
    valid_pred = regr.predict(valid)
    # print("XGBoost score on training set: ", rmse(y_pred, y_test))
    plt.style.use("ggplot")
    plt.title("XGBoost Outcome variables")
    plt.scatter(train_pred, trainlabel, marker="x", color="Blue", label="Train data")
    plt.scatter(valid_pred, validlabel, marker="x", color="Red", label="Validation data")
    plt.plot([10.25, 13.75], [10.25, 13.75], c="Black")
    plt.legend(loc="upper left")
    plt.xlabel("Log of predicted sales values")
    plt.ylabel("Log of actual sales values")
    plt.show()
...
def lassor(train, trainlabel, valid, validlabel):
    trainlabel = np.reshape(trainlabel, len(trainlabel))
    validlabel = np.reshape(validlabel, len(validlabel))
    lasso = LassoCV(alphas=[0.0001, 0.0003, 0.0006, 0.001, 0.003, 0.006, 0.01, 0.03, 0.06,
                            0.1, 0.3, 0.6, 1], max_iter=50000, cv=10)

    lasso.fit(train, trainlabel)
    alpha = lasso.alpha_
    ...
    lasso = lasso.fit(train, trainlabel)
    alpha = lasso.alpha_
    print("Best alpha: ", alpha)
    ly_test = train_price_df
    train_pred = lasso.predict(train)
    valid_pred = lasso.predict(valid)
    # print("Lasso score on training set: " + str(rmse(ly_test, ly_pred)))
    plt.style.use("ggplot")
    plt.title("Lasso regularization")
    plt.xlabel("Log of predicted sales values")
    plt.ylabel("Log of actual sales values")
    plt.scatter(train_pred, trainlabel, c="Black", marker="o", label="Train data")
    plt.scatter(valid_pred, validlabel, c="Blue", marker="s", label="Validation data")
...
# Ridge:
def ridger(train, trainlabel, valid, validlabel):
    rclf = RidgeCV(alphas=[0.01, 0.03, 0.06, 0.1, 0.3, 0.6, 1, 3, 6, 10, 30, 60])
    rclf.fit(train, trainlabel)
    alpha = rclf.alpha_
    print("Best alpha:", alpha)
    ...
    alpha = rclf.alpha_
    print("Best alpha:", alpha)
    train_pred = rclf.predict(train)
    valid_pred = rclf.predict(valid)
    plt.title("Ridge regularization")
    plt.xlabel("Log of predicted sales values")
    plt.ylabel("Log of actual sales values")
    plt.scatter(train_pred, trainlabel, color="Black", marker="s", label="Train data")
    plt.scatter(valid_pred, validlabel, color="Orange", marker="x", label="Validation data")
    plt.legend(loc="upper left")
...
if __name__ == "__main__":
    ...
    Xtrain, Xtest, ytrain, ytest = train_test_split(proc_train_df, train_price_df, test_size=0.3)
    proc_train_df = np.array(proc_train_df)
    print(str(Xtrain.shape))
    print(Xtest.shape)
    print(ytrain.shape)
    print(ytest.shape)
    xgb_pred = xgbr(Xtrain, ytrain, Xtest, ytest)
    # lasso_pred = lassor(Xtrain, ytrain, Xtest, ytest)
    # ridge_pred = ridger(Xtrain, ytrain, Xtest, ytest)
'''
